...
if(strpos($_SERVER['REQUEST_URI'],"loginscript.php") || !isset($_POST['tokserver'])){
...
$check_status = mysqli_query($conn,"SELECT * FROM products WHERE mode = 'script'");
...
$query = $conn->query("SELECT * FROM `tokens` WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'script'");
if($query->num_rows < 1){
    $ackdata = array(
        "app_Status" => "Failed",
...
if($res["UID2"] == "one device"){
		if($res["UID"] == NULL){
			$query = $conn->query("UPDATE `tokens` SET `UID` = '".$data["app_Uid"]."' WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'script'");
	...
	$query = $conn->query("UPDATE `tokens` SET `UID2` = '".$data["app_Uid"]."' WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'script'");
...
$DataFinal = 0;	
if($EndDate < $DataHoje){
	$query = $conn->query("UPDATE `tokens` SET `Expiry` = '".$DataFinal."' WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'script'");
	$ackdata = array(
		"app_Status" => "Failed",
...
if($data["app_Load"] == 1) 
{
    $loaderdata = readFileData("Loaders/script/PUBG.kmods");
    $ackdata = array(
