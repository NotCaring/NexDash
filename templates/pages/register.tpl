        ...
        <main class="flex-column">
          {if $sent}
					...
					<div class="alert alert-success">
						{if $activation}
						Sua conta foi criada com sucesso <strong>{$login}</strong>! Um e-mail com instruções sobre como ativar sua conta foi enviado para seu endereço de e-mail.
						 <meta http-equiv="refresh" content="2; URL='{$script_url}?act=login'"/>
						{else}
						Sua conta foi criada com sucesso <strong>{$login}</strong>! Agora você pode fazer <strong><a href="index.php?act=login&login={$login}">login</a></strong>.
						{/if}
					</div>
					{/if}
				{/if}
            <form class="login" method="POST" action="{$base_url}index.php?act=register">
                <h2 class="text-center">KG<span></span>CHEATS</h2>
                ...
                <div class="d-flex mt-4 flex-column align-items-center">
                  <div class="g-recaptcha" data-sitekey="{$recaptcha_key}"></div>
                    ...
                    <p class="my-3">Já tem uma conta? <a href="{$base_url}index.php?act=login">entrar</a></p>
