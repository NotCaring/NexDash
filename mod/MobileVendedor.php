...
if($res["UID2"] == "one device"){
		if($res["UID"] == NULL){
			$query = $conn->query("UPDATE `tokens` SET `UID` = '".$uid."' WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'injetor'");
		}elseif($res["UID"] != $uid) {

			PlainDie("Dispositivo não permitido.");
		}
} elseif($res["UID2"] == NULL){
	$query = $conn->query("UPDATE `tokens` SET `UID2` = '".$uid."' WHERE `Username` = '".$uname."' AND `Password` = '".$pass."' AND `mode` = 'injetor'");
} elseif($res["UID2"] != $uid) {
