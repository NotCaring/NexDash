...
if (isset($_POST['newuser_form']))
{
	...
	if (count($error) == 0)
	{
		...
		{
			if($device === 1){
				if($row->credits < $config['price']['mod']['1UID']){
					$error[] = 'Créditos insuficientes, são necessários <b>' .$config['price']['mod']['1UID']. '</b> créditos para criar uma conta no modo MOD APK com um aparelho!';
				}
			}else{
				if($row->credits < $config['price']['mod']['1UID']){
					$error[] = 'Créditos insuficientes, são necessários <b>' .$config['price']['mod']['2UID']. '</b> créditos para criar uma conta no modo MOD APK com dois aparelhos!';
				}
			}
	...
	if (count($error) == 0)
	{
		...
		/*if ($mode == 'mod'){
			if($device === 1 ){
				$creditos = $config['price']['mod']['1UID'];
			}else{
				$creditos = $config['price']['mod']['2UID'];
			}
			$modo = 'MOD APK';
		}*/
		...
		if($row->credits >= $creditos){
			...
			if($sth->execute()){
				if($device === 1){
					...
					$dbh = $game_db->prepare('INSERT INTO tokens (Username, Password, StartDate, EndDate, UID2, Expiry, Vendedor, mode) VALUES (?,?,?,?, ?, 15, ?, ?)');
					...
					$dbh = $game_db->prepare('INSERT INTO tokens (Username, Password, StartDate, EndDate, UID2, Expiry, Vendedor, mode) VALUES (?,?,?,?, NULL, 15, ?, ?)');
					$dbh->bindParam(1, $login);
					$dbh->bindParam(2, $password);
					$dbh->bindParam(3, $date);
					$dbh->bindParam(4, $expira);
				...
				$success[] = 'Usuário <strong>' . $login . '</strong> Adicionado no modo <strong>'.$modo.'</strong> Válido por 15 DIAS com conta válida para '.$aparelho.'.';
...
$smarty->assign('mod_1dev', $config['price']['mod']['1UID']);
$smarty->assign('mod_2dev', $config['price']['mod']['2UID']);
